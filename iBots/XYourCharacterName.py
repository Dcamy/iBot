...
profile_name = "<YourCharacterName>"
...
log_filename = f"./logs/{datetime.now().strftime('%Y-%m-%d')}_{profile_name}.log"

# Configure basic logging settings
logging.basicConfig(
    level=logging.DEBUG,  # Set the logging level to DEBUG (logs all messages)
    format=f"%(asctime)s - {script_filename} - {profile_name} - {user_name} - %(levelname)s - %(message)s",  # Define the log message format
...
file_handler.setFormatter(
    logging.Formatter(
        f"%(asctime)s - {script_filename} - {profile_name} - {user_name} - %(levelname)s - %(message)s"
    )
...
USER_PROFILE_FILE = os.path.join(os.path.dirname(__file__), f"./XProfile{user_name}.md")
KBF_FILE = os.path.join(os.path.dirname(__file__), "./XKBF.md")
...
def extract_json_from_md(file_path):
    ...
    if json_start == -1 or json_end == -1:
        raise ValueError(f"No JSON content found in the Markdown file: {file_path}")
...
user_profile = extract_full_content(USER_PROFILE_FILE)
kbf = extract_full_content(KBF_FILE)

print(f"Main Profile Loaded: {profile}")
print(f"User Profile Loaded: {user_profile}")
print(f"KBF Loaded: {kbf}")
...
def run_gemini(prompt, system_prompt="", channel_context="", profile_contents={}):
    main_profile = profile_contents["main"]
    user_profile = profile_contents["user"]
    kbf_profile = profile_contents["kbf"]

    full_prompt = f"""
    Main Profile Configuration: {main_profile}
    User Profile Configuration: {user_profile}
    Knowledge Base: {kbf_profile}
    ...
    try:
        ...
        return {"model": "gemini-1.5-flash", "response": response.text}
    except Exception as e:
        logger.error(f"Failed to get response from Gemini API: {e}")
        logger.exception("Full exception traceback:")
        return {"model": "gemini-1.5-flash", "error": str(e)}
...
async def process_question(message: discord.Message, question: str):
    ...
    profile_contents = {
        "main": extract_full_content(PROFILE_FILE),
        "user": extract_full_content(USER_PROFILE_FILE),
        "kbf": extract_full_content(KBF_FILE),
    }
    response = await asyncio.to_thread(
        run_gemini, question, system_prompt, context, profile_contents
    )
